...
to perform secure string comparison. Double HMAC avoids the timing atacks by blinding the timing channel using random
time per attempt comparison against iterative brute force attacks.
...
**capability urls** so that timing information is not leaked to the attacker.
